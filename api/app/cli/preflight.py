...
def check_configuration(app_env: dict[str, str], api_env: dict[str, str]) -> list[Finding]:
    ...
    if app_audience and api_audience and app_audience != api_audience:
        findings.append(
            Finding("AUTH0_AUDIENCE", "does not match the API's OIDC_AUDIENCE; the token's `aud` would be rejected")
        )
...
def main(argv: list[str] | None = None) -> int:
    ...
    app_env = parse_env_file(root / "web" / ".env.local")
    api_env = parse_env_file(root / "api" / ".env")
